...
func NewRouter(catalogHandler CatalogHandler) *gin.Engine {
	...
	catalogGroup := r.Group(CATALOG_ROUTE)
	{
		...
		catalogGroup.GET(BOOKS_ROUTE+PREVIEW_ROUTE+"/:bookID", catalogHandler.PreviewBook)
		...
		catalogGroup.GET(BOOKS_ROUTE+VIEWS_ROUTE+"/:bookID", catalogHandler.GetBookViewsCount)
